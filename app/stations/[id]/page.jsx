...
const CHART_STYLE = { background: '#0d1520', border: '1px solid #1a2a3a', borderRadius: 4, fontSize: 10 }
const STATUS_HEX  = { danger: '#f43f5e', warning: '#fb923c', safe: '#34d399', info: '#38bdf8' }
const STATUS_CL   = {
...
function ConnectionBanner({ connected, error }) {
  if (connected) return (
    <div className="flex items-center gap-2 px-3 py-1.5 bg-safe/10 border border-safe/30 rounded-lg mb-3">
      ...
      <span className="text-[10px] text-safe font-semibold">WebSocket đã kết nối — Đang nhận dữ liệu thời gian thực</span>
    </div>
  )
  if (error) return (
    <div className="flex items-center gap-2 px-3 py-1.5 bg-warning/10 border border-warning/30 rounded-lg mb-3">
      <span className="text-[10px]">⚠️</span>
      <span className="text-[10px] text-warning font-semibold">{error} — Hiển thị dữ liệu mẫu</span>
    </div>
  )
  return (
...
function MetricCard({ label, value, unit, delta, deltaUp, statusLabel, statusCl, color, data, threshold, stats }) {
  return (
    <div className="bg-card border border-border rounded-xl overflow-hidden flex flex-col"
      style={{ borderTop: `2px solid ${color}` }}>
      <div className="px-4 pt-3.5 pb-0 flex justify-between items-start">
        <div>
          <div className="text-[9px] font-semibold text-muted uppercase tracking-widest mb-1.5">{label}</div>
          <div className="flex items-baseline gap-1.5">
            <Mono className="text-[26px] font-bold leading-none" style={{ color }}>{value}</Mono>
            <span className="text-[11px] text-muted">{unit}</span>
            {delta !== null && (
              <span className={`text-[10px] font-semibold ${deltaUp === true ? 'text-danger' : deltaUp === false ? 'text-safe' : 'text-muted'}`}>
                {deltaUp === true ? '↑' : deltaUp === false ? '↓' : '—'} {delta}
              </span>
            )}
          </div>
        </div>
        <span className={`text-[8px] font-bold tracking-wide px-2 py-0.5 rounded-full border mt-1 whitespace-nowrap ${statusCl}`}>
          {statusLabel}
        </span>
      </div>
      ...
      <div className="px-1 pt-1" style={{ height: 76 }}>
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 4, left: -38, bottom: 0 }}>
            <defs>
            ...
            <Tooltip contentStyle={CHART_STYLE}
              ...
              formatter={v => [`${v} ${unit}`, '']} />
            {threshold != null && (
              <ReferenceLine y={threshold} stroke="#f43f5e" strokeDasharray="3 3" strokeWidth={1} />
            )}
      ...
      <div className="grid grid-cols-3 border-t border-border mt-auto">
        {stats.map(({ lb, val, cl }, i) => (
          <div key={lb} className={`px-3 py-2.5 ${i < 2 ? 'border-r border-border' : ''}`}>
...
export default function StationDetailPage() {
  ...
  const waterChartData = useMemo(() => {
    if (history?.waterLevel?.length) return historyToChartData(history, 'waterLevel')
    // fallback: flat mock line
    return Array.from({ length: 20 }, (_, i) => ({ t: `${i}:00`, v: +(waterLevel - 1 + i * 0.08).toFixed(2) }))
  }, [history, waterLevel])

  const humidChartData = useMemo(() => {
    if (history?.moisture?.length) return historyToChartData(history, 'moisture')
    return Array.from({ length: 20 }, (_, i) => ({ t: `${i}:00`, v: +(moisture - 5 + i * 0.3).toFixed(1) }))
  }, [history, moisture])

  const vibChartData = useMemo(() => {
    if (history?.freq?.length) return historyToChartData(history, 'freq')
    return Array.from({ length: 20 }, (_, i) => ({ t: `${i}:00`, v: +(2 + Math.sin(i / 3) * 1.5).toFixed(2) }))
  }, [history, freq])
  ...
  // Status
  const waterSt = getWaterStatus(waterLevel, st.bd3, st.bd2, st.bd1)
  const humidSt = getMoistureStatus(moisture)
  const vibSt   = getVibrationStatus(freq)
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex items-center gap-1.5 mb-3 text-[11px]">
        <Link href="/" className="text-muted no-underline hover:text-tx">Trang chủ</Link>
        <span className="text-muted">›</span>
        <Link href="/stations" className="text-muted no-underline hover:text-tx">Danh sách trạm</Link>
        <span className="text-muted">›</span>
      ...
      <div className="flex justify-between items-start mb-5">
        <div>
          <div className="flex items-center gap-2.5 mb-1.5">
            <h1 className="text-xl font-bold text-tx tracking-wide m-0">{st.river} — {st.km}</h1>
            <Badge status={waterSt.level} />
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-1.5 h-1.5 rounded-full animate-pulse-dot ${stStatus.dot}`} />
            <span className="text-[10px] text-muted">
              {st.name}
              {latest?.timestamp && (
                <> • Cập nhật: {new Date(latest.timestamp).toLocaleTimeString('vi-VN')}</>
              )}
            </span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-3">
        ...
        <MetricCard
          label="Mực nước"
          value={waterLevel.toFixed(2)} unit="m"
          ...
          stats={[
            ...
            { lb: 'Đỉnh 24h',   val: `${waterStats.max}m`,  cl: `text-${waterSt.level === 'danger' ? 'danger' : 'warning'}` },
        ...
        <MetricCard
          ...
          stats={[
            ...
            { lb: 'Ngưỡng',     val: '85%',                  cl: 'text-warning' },
          ]}
        />
        ...
        <MetricCard
          ...
          stats={[
            ...
            { lb: 'Ngưỡng',     val: '8.0 Hz',               cl: 'text-muted'   },
          ]}
        />
      </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-card border border-border rounded-xl p-4">
            <div className="flex justify-between items-start mb-3">
              <div>
                <div className="text-[12px] font-semibold text-tx">Biên độ rung & Lưu lượng</div>
                <div className="text-[9px] text-muted mt-0.5">Dữ liệu cảm biến thời gian thực</div>
              </div>
              {latest && (
            ...
            <div className="grid grid-cols-2 gap-2 mb-3">
              {[
                { lb: 'Biên độ rung',  val: `${amp.toFixed(2)} mm`,               sub: 'Amplitude sensor',       cl: 'text-accent'  },
                { lb: 'Mức chứa',      val: `${percent.toFixed(1)}%`,              sub: 'Theo mực nước hiện tại', cl: 'text-info'    },
                { lb: 'Tần số rung',   val: `${freq.toFixed(2)} Hz`,               sub: 'Vibration frequency',    cl: 'text-accent'  },
                { lb: 'Lưu lượng TK',  val: `${st.flow.toLocaleString()} m³/s`,   sub: 'Mock — chưa có sensor',  cl: 'text-muted'   },
              ].map(({ lb, val, sub, cl }) => (
            ...
            <div>
              <div className="flex justify-between text-[9px] text-muted mb-1">
                ...
                <Mono className="text-info">{percent.toFixed(1)}%</Mono>
              </div>
              <div className="h-2 bg-border rounded-full overflow-hidden">
                <div className="h-full rounded-full transition-all duration-700"
                  style={{
                    width: `${Math.min(percent, 100)}%`,
                    background: percent > 90 ? '#f43f5e' : percent > 75 ? '#fb923c' : '#34d399'
                  }} />
              </div>
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-xl p-4 flex-1">
            <div className="flex justify-between items-center mb-3">
              <span className="text-[12px] font-semibold text-tx">Cảnh Báo & Sự Kiện</span>
              <span className="text-[10px] text-accent cursor-pointer font-semibold hover:underline">Xem tất cả</span>
            </div>
            {[
              { icon: '⚠️', bg: 'bg-warning/10', title: 'Mực nước cảnh báo cấp 1', time: '10P',
                desc: `Trạm ${st.km} vượt ngưỡng BĐ1. Kích hoạt theo dõi tăng cường.` },
              { icon: '✅', bg: 'bg-safe/10',    title: 'Bảo trì hoàn tất',         time: '2H',
                desc: 'Đã kiểm tra cảm biến độ rung & độ ẩm số 2. Kết nối ổn định.' },
              { icon: '🌊', bg: 'bg-info/10',    title: 'Biến động dòng chảy',      time: '5H',
                desc: 'Phát hiện thay đổi tần số rung bất thường tại thượng lưu 500m.' },
            ].map(({ icon, bg, title, time, desc }, i) => (
              <div key={i} className="flex gap-2.5 mb-3 last:mb-0">
                <div className={`w-7 h-7 rounded-full ${bg} flex items-center justify-center shrink-0 mt-0.5 text-sm`}>
                  {icon}
                </div>
                <div>
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="text-[11px] font-semibold text-tx">{title}</span>
                    <Mono className="text-[8px] text-muted">{time} TRƯỚC</Mono>
                  </div>
