...
export async function createSensorCluster(data, token = null) {
  ...
  const json = await res.json()
  if (!res.ok) throw new Error(json.message || `POST /sensor-clusters failed: ${res.status}`)
  return json
}
...
export async function updateSensorCluster(id, data, token = null) {
  ...
  const json = await res.json()
  if (!res.ok) throw new Error(json.message || `PUT /sensor-clusters/${id} failed: ${res.status}`)
  return json
}
...
export async function deleteSensorCluster(id, token = null) {
  ...
  const json = await res.json()
  if (!res.ok) throw new Error(json.message || `DELETE /sensor-clusters/${id} failed: ${res.status}`)
  return json
}
...
export async function addSensorDevice(clusterId, data, token = null) {
  ...
  const json = await res.json()
  if (!res.ok) throw new Error(json.message || `POST /sensor-clusters/${clusterId}/devices failed: ${res.status}`)
  return json
}
...
export async function updateSensorDevice(clusterId, deviceId, data, token = null) {
  ...
  const json = await res.json()
  if (!res.ok) throw new Error(json.message || `PUT device ${deviceId} failed: ${res.status}`)
  return json
}
...
export async function deleteSensorDevice(clusterId, deviceId, token = null) {
  ...
  const json = await res.json()
  if (!res.ok) throw new Error(json.message || `DELETE device ${deviceId} failed: ${res.status}`)
  return json
}
