...
// ── Backward Compatibility Wrappers for Sensor Clusters (Cached) ──
export async function fetchSensorClusters(stationId, damId, forceRefresh = false) {
  try {
    const data = await fetchNodes(undefined, stationId, damId, forceRefresh)
    let nodes = data.nodes || []

    if (stationId) {
      nodes = nodes.filter((n) => {
        const sId = n.gateway?.stationId || n.stationId || n.gateway?.station?.id
        return Number(sId) === Number(stationId)
      })
    }

    if (damId && damId !== 'all') {
      nodes = nodes.filter((n) => {
        const dId = n.gateway?.station?.damId || n.gateway?.station?.dam?.id || n.damId
        return String(dId) === String(damId)
      })
    }
...
export async function fetchSensorClusterById(id, forceRefresh = false) {
  const res = await fetchNodeById(id, forceRefresh)
